...
#code for table deletion
@app.route('/rest/api/v1/urlScanner/deletetable', methods=['DELETE'])
def delete_table():
    if flask.request.method == 'DELETE':
        # response_string = 'got all records'
        response_string = dblayer.delete_table()
        return Response(response_string, status=200)
    else:
        response_string = 'Only DELETE method is supported with this endpoint'
        return Response(response_string, status=405)
...
def is_safe_url():
    if flask.request.method == 'GET':
        ...
        if hostname is not None and url is not None:
            ...
            if port is not '':#changed none to empty string
...
if __name__ == '__main__':
    dblayer.create_table()#calling create table method
