...
def create_table():
    try:
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS url_db ( id int NOT NULL AUTO_INCREMENT, URL varchar(10000) NOT NULL, PRIMARY KEY (id))")
        mydb.commit()
    except Exception as e:
        print(' Exception occurred in table creation{}'.format(e))
...
def delete_table():
    try:
        mydb = start_db()
        cursor = mydb.cursor(dictionary=True)
        cursor.execute("DROP TABLE url_db")
        mydb.commit()
        return "table deleted succesfully"
    except Exception as e:
        print(' Exception occurred in table deletion{}'.format(e))
        return 'Exception occurred'
    finally:
        cursor.close()
        mydb.close()
